...
def answer_two():
    ## max(abs(df['Gold']-df['Gold.1'])) kinda goes down a rabbit hole...
...
def answer_three():
    only_golds = df.where((df['Gold'] > 0) & (df['Gold.1'] >0))
    only_golds = only_golds.dropna()
    big_gold_dif = (abs((only_golds['Gold']-only_golds['Gold.1']))/(only_golds['Gold']+only_golds['Gold.1'])).argmax()
    return  big_gold_dif
