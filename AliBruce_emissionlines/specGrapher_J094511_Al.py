...
for i in range(len(tfiles)):
	...
	ax1.plot(data[:,0], medfilt(data[:,1], kernel_size=kernels[i])/10**-17, linewidth=0.5)
	ax2.plot(data[:,0], medfilt(data[:,1], kernel_size=kernels[i])/10**-17, linewidth=0.5)
